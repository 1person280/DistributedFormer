...
### 新增
- **节点内联控件 (widget)**：参数以滑块/数字/文本框/下拉直接绘制在节点 body 上，
  与 `n.params` 及右侧属性面板双向实时同步。
- **左侧浮动侧栏**：顶部 Generate + 队列数量徽章、Interrupt 中断 / 清空队列按钮；
  底部 `Queue`/`Load`/`Explorer` 三个标签页（执行队列与最近运行 / 模板与本地工作流 /
  模型与插件浏览器，点击即加载或加节点）。`Ctrl+B` 或工具栏 ☰ 折叠/展开。
- **节点增强**：标题栏类别色条、运行时长徽章、成功/失败/执行中状态色边框、逐节点按
  拓扑顺序点亮的渐进式执行动画、旁通 (Bypass)/禁用 (Mute)、直通 (Reroute) 节点、
  备注 (subtitle) 注释。三者均可右键或属性面板切换。
- **选区组 (Group)**：框选/多选后 `Ctrl+G` 或右键"转为组"；整组拖动/重命名/折叠/禁用；
  组信息随工作流保存与导出。
- **拖放**：把 `.json` 工作流文件或多个模型文件直接拖入画布（高亮 HUD）即可加载/加节点。
- **更多快捷键**：`Ctrl+A` 全选、方向键（+Shift 加速）微移选中节点、`Ctrl+G` 建组、
  `Ctrl+B` 侧栏、`Ctrl+Enter` 入队运行、`Esc` 中断提示。

### 变更
- 运行升级为**客户端队列**：可排队执行、显示队列数量徽章，逐节点高亮与日志随拓扑推进。
- 版本号 0.15.x → 0.16.0（pyproject / `__version__` / `pkg.CORE_VERSION` /
  `min_core_version` / `web_ui._UI_VERSION` / `workflow_ui._VERSION`）。

### 测试
- `test_workflow_ui.py`（5）、`test_interplugin.py`（12) 全通过；页面 HTTP 冒烟验证
  （/workflow 含侧栏/inline widget；load→run 端到端 ok）。
...
Llama 等）及**同为脉冲/事件驱动的 SNN 框架**（snnTorch / Norse / Lava）做能力/架构对照，
并附**实测基准对比**（纯真实数据，同行未实测处如实标注，不编造精度）。纯文档 + 版本号变更。

### 变更
- **README 对比一览**：能力矩阵（定位 / 基本单元 / 事件驱动 / 持久工作记忆 / 参数量 /
  运行环境 / 插件热加载 / 内置安全监控 / 持续在线学习）+ 4 真实任务实测基准
  （5 种子 × 5 折，同行对标列如实留待硬对比）。
- **版本命名彩蛋**：注明 `v0.14.0 —— 向祖传代码开炮` 致敬手游《向僵尸开炮》。
- **版本号**：0.14.1 → 0.14.2（pyproject / `__version__` / `pkg.CORE_VERSION` /
  `min_core_version`）。
...
> **命名彩蛋**：`v0.14.0 —— 向祖传代码开炮` 致敬手游《向僵尸开炮》——祖传/遗留代码即
> 这一版要"消灭的僵尸"，纯命名玩梗，不影响版本语义。
