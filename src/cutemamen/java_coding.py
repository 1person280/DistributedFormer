"""JavaCodingPlugin: Java coding 思考插件 (进军祖传代码)

v0.14.0 起插件携带**主模型迁移知识** (分布式架构, 类比 RustCodingPlugin):

    主模型 (冻结 CubeGPT 水库 + 线性读出层, training/readout.py)
        ──migrate_from_main_model()──▶  插件可存档权重 (W/mean/scale)
        ──.CuteMamen 包──▶ 内核路由即服务 (宿主无需主模型)

知识迁移与主模型验证协议逐位一致: 真实 Java 祖传代码语料 → 双模态
注入 (numeric=static+structure 语法/结构特征, text=代码原文) → 冻结
CubeGPT 水库特征 → 线性 softmax 读出层。迁移后 on_think 优先走读出
层; 无读出权重时回退到语料蒸馏原型 (最近质心)。

route = "java": on_think 把一段 Java 代码分类到 8 类静态问题之一
(null/rawtype/deprecated/generic/type/symbol/override/ok), 并把结果
发布到事件总线 ("java.classified") 供其他插件订阅。
"""
...
# 相对导入真实语料 (src.data.java_coding)
try:
    from ..data.java_coding import (
        LABELS, JAVA_SNIPPETS, static_metrics, structure_metrics)
except Exception:  # pragma: no cover - 极罕见时序问题, 见 _ensure_corpus
    LABELS, JAVA_SNIPPETS = [], []
    static_metrics = structure_metrics = None


class JavaCodingPlugin(ExpertPlugin):
    """静态识别一段 Java 代码命中哪一类祖传代码问题的思考插件

    训练材料: 内嵌 294 段真实 Java 代码 + 真实静态问题类别 (null /
    rawtype / deprecated / generic / type / symbol / override / ok)。
    原型权重 = 每类别 static_metrics 均值; 分类 = 最近原型 (特征空间
    欧氏距离), 置信度 = softmax(-距离)。

    事件格式: {"topic": "java", "data": <代码 str 或 {"code": str}>}
    返回: {"label", "label_name", "javac", "confidence"},
    并广播 "java.classified"。
    """

    BASE_MODEL = "java.coding"
    CAPABILITY = ("静态识别 Java 祖传代码命中的静态问题族 "
                  "(null/rawtype/deprecated/generic/type/symbol/"
                  "override/ok); v0.14.0 起可携带主模型 (CubeGPT 水库"
                  "读出层) 迁移知识")

    def __init__(self, name: str = "java-coding", *,
                 route: Optional[str] = None, **kwargs):
        super().__init__(name, route=route or "java", **kwargs)
    ...
    def _ensure_main_model_bridge(self) -> None:
        ...
        if self._dataset is None:
            self._dataset = JavaCodingTrainingDataset(dim=cfg["dim"])
    ...
    def classify(self, code: str) -> Dict[str, Any]:
        """公开分类入口: 一段 Java 代码 → 命中的静态问题族

        推理路径: 主模型迁移读出层 (有迁移权重时) → 语料原型回退。
        返回 {"label","label_name","javac","confidence","source"}。
        """
        self._ensure_corpus()
        ...
        if self.readout is not None:
            ...
            return {"label": "ok", "label_name": "合法代码 (可编译)",
                    "javac": "-", "confidence": 0.0, "needs_corpus": True,
                    "source": "none"}
        return {"label": label, "label_name": _label_name(label),
                "javac": _javac_for(label), "confidence": confidence,
    ...
    def on_think(self, event: Dict[str, Any],
        ...
        if ctx is not None:
            ctx.working_memory.push(
                f"java_{ctx.kernel_version}_{len(ctx.working_memory.recent_events)}",
                f.astype(float), np.array([confidence]))
            ctx.emit("java.classified",
...
def _javac_for(label: str) -> str:
    return _JAVAC_BY_LABEL.get(label, "-")
