...
from src.codec.class_token import (
    CHAR_GROUP, LOW32_MASK, MAX_TOK, UNICODE_MAX,
    make_token, group_of, low_of, is_char_token,
    char_token, char_group_token, encode, tokenize, decode, embed_tokens,
)
...
def test_low32_is_codepoint():
    text = "H 汉 🔥 #"
    for tok, ch in zip(encode(text), text):
        assert low_of(tok) == ord(ch)
