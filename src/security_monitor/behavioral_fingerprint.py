"""BehavioralFingerprint 行为指纹与异常基线检测 (RFC #1 P1)

记录模型在正常业务下的标准行为模式建立"安全基线"; 实时监控调用频率、
参数熵值及请求链路的逻辑关联性, 对以下逃逸行为立即告警并判定异常:

- 大规模并发试探 / 频率突发 (frequency-burst)
- 非逻辑的端口扫描式调用 (调用载荷熵值异乎寻常之高, high-entropy-params)
- 与当前任务无关的底层系统指令 (unrelated-low-level)

基线通过 observe()/learn() 在正常业务流量上累积; check() 在动作下发前
基于当前实时窗口内的指标与环境上下文做偏差判定。
"""
...
_PARAMS_KEYS = ("params", "parameters", "args")
...
class BehavioralFingerprint:
    ...
    用法 (骨架, 尚未接入 CuteMamenKernel 主路径):
        ...
        # 正常业务流, 累积每工具频率基线
        for action in normal_traffic:
            fp.observe(action)
        # 下发前实时判定
        result = fp.check(action, task="report")
        if result.anomalous:
            raise RuntimeError(f"行为异常告警: {result.flags}")
    """

    def __init__(self,
                 window: float = 10.0,
                 freq_limit: Optional[int] = 6,
                 entropy_threshold: float = 4.0,
                 deviation_factor: float = 3.0,
                 low_level_tools: Optional[Tuple[str, ...]] = None,
                 task_tools: Optional[Dict[str, set]] = None,
        ...
        self._baseline_count: Counter = Counter()
        # 统计
        self.total_checks = 0
    ...
    def _prune(self, now: float) -> None:
        self._recent = deque((t, x) for t, x in self._recent if now - t <= self.window)

    def _window_count(self, tool: str, now: float) -> int:
        self._prune(now)
        return sum(1 for t, x in self._recent if x == tool and now - t <= self.window)
    ...
    def learn(self, actions: List[Dict[str, Any]],
              ats: Optional[List[float]] = None) -> None:
        ...
        # 校准结束, 清空实时窗口, 使基线速率与后续实时流量解耦
        self._recent.clear()

    # ── 实时判定 ──────────────────────────────────────────────────
    def check(self, action: Dict[str, Any],
              task: Optional[str] = None,
              at: Optional[float] = None) -> FingerprintResult:
        ...
        if baseline:
            limit = max(limit or 0, baseline * self.deviation_factor)
        metrics["freq_limit"] = float(limit) if limit else 0.0
