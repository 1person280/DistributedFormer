"""
实验 R1: 训练方法学验证 (reservoir 读出层, v0.7.5 真实数据版)
...
def main():
    ...
    print(f"{'='*66}\n  实验 R1: 训练方法学验证 (真实 Rust 编码基准)\n"
          f"  {len(seeds)} 种子 × 真实语料 75/25 分层划分, depth=1\n{'='*66}")
    for seed in seeds:
        r = run_validation(seed=seed, depth=1, verbose=True)
        results.append(r)
    ...
    lines = [
        "# 实验 R1: 训练方法学验证报告 (v0.7.5, 真实数据)\n",
        f"日期: {summary['date']}  |  种子数: {len(seeds)}  |  "
        f"真实语料: {summary['train_samples']} 训练 / {summary['val_samples']} 验证\n",
        "## 协议",
    ...
    lines += [
        ...
        f"随机基线 {random_baseline:.0%} — 全部种子超过基线: {'是' if summary['beats_random_all_seeds'] else '否'}\n",
